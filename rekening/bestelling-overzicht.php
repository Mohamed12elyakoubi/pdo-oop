...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['fetchReservationData'])) {
        ...
        exit;
    } elseif (!empty($_POST['ReserveringID']) && !empty($_POST['klant_id'])  && !empty($_POST['tafel_id']) && !empty($_POST['product_id']) && !empty($_POST['omschrijving']) && !empty($_POST['datum']) && !empty($_POST['tijd']) && !empty($_POST['aantal']) && !empty($_POST['Prijs']) && !empty($_POST['totaal_prijs'])) {
        $ReserveringID = $_POST['ReserveringID'];
        ...
        $rekening->insertrekening($ReserveringID, $klant_id, $tafel_id, $product_id, $omschrijving, $datum, $tijd, $aantal, $Prijs, $totaal_prijs);
...
<body>
    ...
    <div class="container">
        ...
        <table class="table">
            <thead>
                <tr>
                    <th class='table-light'>bestelling_nummer</th>
                    ...
                    <th class='table-light'>Prijs per stuk </th>
                    <th class='table-light'>totaal Prijs</th>
        ...
        <div class="container">
            ...
            <form method="POST" action="">
                    ...
                    <div class="input-group">
                        <select class="form-control" name="product_id" id="product_id">
                            <?php foreach ($product as $productItem) : ?>
                                <option value="<?php echo $productItem['productid']; ?>"><?php echo $productItem['productid'];
                                                                                            echo '---';
                                                                                            echo $productItem['product'];  ?> </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="form-group">
                    ...
                    <button type="button" class="btn btn-outline-secondary" onclick="calculateTotal()">Bereken totale prijs</button>

                </div>

                <div class="form-group">
                    <label for="totaal_prijs">Totale Prijs:</label>
                    <input type="text" class="form-control" name="totaal_prijs" id="totaal_prijs" readonly>
                </div>
                <br>
        ...
        var aantal = parseFloat(document.getElementById('aantal').value);
        var prijs = parseFloat(document.getElementById('Prijs').value);
        var totaal_prijs = aantal * prijs;
        if (!isNaN(totaal_prijs)) {
            document.getElementById('totaal_prijs').value = totaal_prijs.toFixed(2);
        } else {
            document.getElementById('totaal_prijs').value = '';
        }
    }
        </script>
</body>
